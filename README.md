...
ClipForge is a local Windows capture app with two independent modes: leave a private rolling Instant Replay buffer running and save a moment that already happened, or manually start Recorder and stop it whenever the full session should be saved. ClipForge keeps capture local while providing complete in-app playback controls, a customizable dark interface, conservative capture optimizations, and tightly scoped media-library and update handling.
...
- A manual full-session Recorder that runs from **Start recording** until **Stop & save**. Recorder and Instant Replay are mutually exclusive and never stack capture processes or overlays.
...
- Toggle a compact transient replay overlay with a second configurable shortcut (**Ctrl+Shift+F9** by default). It is topmost only while shown and dismisses itself automatically.
...
- Optionally start Recorder instead after Windows sign-in. The Replay and Recorder startup choices are mutually exclusive.
...
dotnet run --project .\tests\ClipForge.CaptureSmoke\ClipForge.CaptureSmoke.csproj -c Release -- --length-matrix
dotnet run --project .\tests\ClipForge.CaptureSmoke\ClipForge.CaptureSmoke.csproj -c Release -- --recorder-service-smoke --resolution 1080p --fps 60 --audio --save-seconds 12 --force-segment-fallback
...
dotnet run --project .\tests\ClipForge.CaptureSmoke\ClipForge.CaptureSmoke.csproj -c Release -- --wgc-matrix --matrix-resolutions source,720p,1080p,1440p,2160p --fps 60 --motion-validation --self-motion-surface
...
Run `--wgc-matrix --motion-validation --self-motion-surface` for a repeatable composed-desktop check: the parent matrix covers the captured display with a topmost animated surface, waits until its first frame is visible, and always stops it after the run. Omit `--self-motion-surface` when testing a real game with its fullscreen/custom mode already selected. The controlled surface does not reproduce every affected GPU, driver, exclusive-fullscreen path, or game's internal resolution; target-PC game testing remains required.

NuGet restore may use the network the first time the project is built.

## First-run FFmpeg setup

ClipForge uses FFmpeg as its local capture and MP4 engine. If it cannot find a complete verified `ffmpeg.exe` and sibling `ffprobe.exe` pair, the app shows an **Install engine** panel. Selecting **Install engine** downloads the FFmpeg essentials archive over HTTPS from Gyan.dev, verifies both tools, and publishes them together in:
...
For a manual full-session recording, leave Instant Replay off and select **Start recording**. Recorder ignores the Replay-length selector and continues until **Stop & save** is selected (or until the storage safety policy must stop it). It writes one seekable fragmented MP4 continuously and keeps separate recovery checkpoints, so a normal stop does not remux the complete recording; very large sessions therefore take roughly the same bounded close time as short sessions. Recorder requires a fixed local drive with at least 4 GiB free at start, and capture-time storage can be close to twice the eventual MP4 because the independent recovery stream is retained until commit. If ClipForge or Windows exits abnormally, the next launch offers the preserved session for retry instead of silently starting over.
...
To make capture available automatically after restarting or signing back into Windows, enable either **Start ClipForge and replay with Windows** or **Start Recorder with Windows** in the settings sidebar. These explicit opt-ins are mutually exclusive and off by default. The selected option creates a per-user Windows Startup shortcut; at the next sign-in ClipForge launches in the background, finishes loading the saved capture configuration and local FFmpeg engine, and starts only that mode. If initialization or the engine is not ready, ClipForge does not force a capture start. A normal manual launch still opens the main window, and disabling both settings removes the Startup shortcut.

The saved MP4 goes to the selected folder and becomes available in the player and recent-clips gallery. After the save succeeds, ClipForge can play a short confirmation sound and show a compact in-app confirmation; disable the sound at any time from **Feedback**. The player includes a timeline, elapsed/total time, play/pause, restart, 10-second skip controls, mute, and volume. Saving does not stop the rolling buffer, so another clip can be saved later. Before and during a save, ClipForge reserves and monitors space for both the complete MP4 and the rolling segments that must remain protected; a full separate output drive cancels only that save, while a full buffer drive stops replay safely. Stopping replay clears the temporary buffer. Changing the display, resolution, frame rate, or audio configuration while replay is active automatically restarts capture and clears the old buffer. Changing only replay length updates the visible retained duration immediately without restarting capture; excess files are pruned asynchronously in bounded batches instead of synchronously blocking the settings UI. If Windows reports that the selected display changed, ClipForge refreshes it by device name. Direct Windows Graphics Capture keeps the existing buffer for coordinate-only monitor moves, but now reacquires its frame pool whenever the monitor's source dimensions change, even when a fixed preset still resolves to the same encoded size. Source/output-size changes and GDI's baked coordinates or input dimensions also restart safely. If a mode transition faults capture before the refresh completes, ClipForge remembers that replay was requested and recovers it with the newest geometry.

The gallery automatically loads only top-level files using ClipForge's generated normal `Clip_YYYY-MM-DD_HH-mm-ss[_N].mp4` and trimmed `Clip_YYYY-MM-DD_HH-mm-ss[_N]_trimmed[_N].mp4` forms. Other MP4 files in the save folder are left untouched and are not automatically decoded by the embedded player. Use the selector above the gallery to show 4, 8, 10, or 15 recent clips; the available clips fill the row, every card shows its size, and larger sets scroll horizontally. Select **Library** to browse up to the newest 100 validated recordings in a recycling list and filter the view to All, Normal, or Trimmed clips. Clicking any Library entry loads it into a large local player with play/pause, restart, timeline seeking, 10-second skips, mute, and volume. During replay, normal automatic folder discovery and FFprobe metadata work stay suppressed. A hidden Windows-autostart launch validates and binds the requested Recent snapshot before capture begins. If that bounded preload times out or fails, a visible and active Main window can recover missing cards while capture is steadily Ready through a two-second cached-poster/metadata-only refresh; it never starts new thumbnail generation and retries no more often than every 30 seconds. The same foreground Ready state may fill missing posters for already identity-bound cards with one bounded Idle-priority helper at a time. Focus loss or a start/save/stop transition cancels either task immediately. Explicit playback, seeking, and trimming of an existing selected clip remain available, and replay-time playback starts muted. Stopping replay resumes a full Recent/Library refresh and fills missing metadata and previews. Unmuting is allowed, but its sound is normal desktop audio and can therefore enter the rolling buffer when desktop audio capture is enabled. The short start, save, and stop transitions temporarily suspend presentation work and release the decoder, then restore the selected clip when the transition finishes. Right-clicking a Recent or Library item can reveal the revalidated file in Explorer or permanently delete that exact ClipForge recording after confirmation.

To trim, open the latest clip or a Library clip, enable the trim editor, and move its two handles to the first and last frame you want to keep. ClipForge verifies an aligned start against the actual H.264 keyframes and uses stream copy when that result is safe; an unreadable or timeline-invalid copy retries through exact encoding. Other ranges use a frame-accurate hardware encode with a software fallback. Both stopped and steady-replay modes run the explicit trim below foreground-game priority without artificial real-time pacing. ClipForge validates the completed MP4 and then adds it to the Library as a separate trimmed clip. The normal clip is kept by default. Only after a successful export does ClipForge ask whether to delete it; accepting that prompt still revalidates the exact original file identity before deletion.
...
Closing the main window hides it to the notification area rather than stopping capture. The ClipForge process and FFmpeg capture process must remain running for instant replay to work; choose **Exit ClipForge** from the tray menu when you want them to stop. The compact overlay is a desktop control surface, not an injected in-game overlay. Pointer clicks use Windows mouse-activation suppression so they do not intentionally steal focus or relative-mouse ownership from a game, while the controls remain available to accessibility navigation. It is topmost only while explicitly visible, hides after ten idle seconds, and enforces a 15-second hard limit so a stuck pointer capture cannot leave it in the fullscreen composition path. Use the overlay shortcut to show the same window again. Exclusive-fullscreen applications may still display above it.

## Capture performance

At replay startup, ClipForge runs short, real encoding probes instead of assuming that an encoder compiled into FFmpeg is usable with the installed driver. For a uniquely mapped, unrotated monitor it first verifies clocked Desktop Duplication with NVIDIA NVENC, Intel Quick Sync, or AMD AMF. Desktop Duplication continues producing frames when the desktop is unchanged, so a quiet screen does not stall the startup check or the rolling buffer. Windows Graphics Capture (WGC) remains a compatibility path when Desktop Duplication is unavailable. A GDI fallback is accepted only after its real three-second `gdigrab`/scale/encoder graph also sustains the requested output cadence; software H.264 remains the final encoder fallback and must pass a production-shaped check. The active strategy is shown in the interface.

Desktop Duplication explicitly binds the selected monitor's DXGI adapter and adapter-local output, rather than reusing the unrelated WinForms monitor ordinal. Native capture avoids resizing; NVIDIA downscaling uses the existing CUDA path, with a separately probed system-memory compatibility path. Quiet-frame repetition occurs inside the capture source, so its diagnostics report output throughput rather than claiming to measure distinct desktop updates. Throughput, progress-gap, and bounded process-failure recovery remain enabled. Adapter/output identity and geometry changes reacquire the source; rotated or unresolved outputs use the verified compatibility paths. These changes do not alter the one-hour Replay limit or manual Recorder duration.

Source/native WGC, already-fitting presets, and native GDI begin at BelowNormal CPU/GPU priority; native WGC uses a two-frame input queue. A fixed-preset WGC or GDI path that actually downsizes uses Normal CPU/GPU priority, while scaled WGC also uses a four-frame queue so its capture/D3D scaling graph can sustain cadence on stretched or custom-resolution games. If native LowImpact capture proves repeated subsecond stalls, a multi-second progress gap, below-real-time output, or a sustained low cadence from process startup while a fullscreen game has continuing input, ClipForge performs one non-looping promotion to the resilient profile. Already-promoted and scaled paths do not use the aggressive burst/low-cadence tier; persistent objective throughput failure enters bounded recovery instead. A proven native profile is retained across automatic custom-mode display restarts until the user starts a new replay session. Every WGC and fallback-GDI capability probe now exercises the full three-second production-shaped CFR graph with the queue and scheduling profile that production will use; the policy is reapplied after graph initialization and repaired every 30 seconds. Probe cache keys include the selected display/device coordinates, cursor graph, geometry, frame rate, and performance profile. A temporary failed WGC probe is cached for 15 seconds with exponential growth capped at one minute, while an active GDI fallback rechecks WGC on a deliberately quieter five-, ten-, twenty-, then thirty-minute schedule. That real capture/encode recheck is deferred while there is recent foreground input, is cancelled if input resumes during the probe, and runs only after a stable idle period. Successful WGC selections expire after ten minutes, and a confirmed real launch failure invalidates only the exact selection before one fresh retry.

ClipForge is Per-Monitor-V2 DPI aware so display coordinates and WGC source pixels remain consistent on mixed-scaling systems. Fixed presets downscale to the smallest even aspect-preserving size inside their bound, bypass scaling when the source already fits, and never pay for a padded fixed canvas. NVIDIA WGC keeps an even native capture surface, crosses the D3D11/CUDA boundary through bounded system memory, and then performs the nearest-neighbour CUDA reduction; compatibility-transfer paths use system-memory fast-bilinear scaling, while directly compatible QSV/AMF paths keep the runtime-probed D3D11 point sampler. **Source** follows the native capture surface, subject only to encoder-safe even dimensions, and therefore avoids preset downscaling. Other background probes and thumbnails run at Idle priority, while an explicit trim runs at BelowNormal. Desktop/microphone PCM transfer is bounded and reuses pooled buffers so capture cannot grow an unlimited in-memory queue. Replay retention follows FFmpeg's sequential segment names incrementally instead of repeatedly enumerating and sorting the entire buffer. Valid unchanged clip metadata and short-lived negative probe results are reused from bounded caches, corrupt thumbnail files must pass a real bounded JPEG decode before reuse, and frozen thumbnail decodes use a strong LRU bounded by both 48 keys and approximately 32 MiB of raw pixels. Hidden, inactive, and tray states cancel unnecessary helpers, release embedded-player sources, keep at most one decoder active, and avoid rebuilding the full UI on background state ticks. Recent and Library bind validated cached cards immediately, then hydrate missing posters through cancellable bounded work. Throughout Buffering and Ready, normal automatic folder discovery and FFprobe work remain suspended; only the bounded autostart recovery described above may refresh missing Recent metadata, and only an active foreground Main window may hydrate missing posters for its validated Recent snapshot. Cached cards stay visible and explicitly requested playback or trimming remains available. A full refresh resumes after replay stops. Player position timers run only while their visible, active window is playing a clip. NVIDIA capture favors game headroom with the fast P2 preset, single-pass encoding, no lookahead, and no B-frames. Capture, save, validation, thumbnail, probe, and trim children are attached to kill-on-close Windows Job Objects so an abrupt ClipForge process termination does not intentionally leave owned media helpers running.

ClipForge retains FFmpeg progress reports at quarter-second cadence. WGC watches for an alive process whose progress/segments stop, a confirmed 2.5-second-or-longer progress gap, below-real-time output, repeated short stalls, severe eight-second duplicate starvation, chronic startup low cadence in the initial native profile, and moderate twenty-second degradation after an aged process has already demonstrated healthy active cadence. A delayed first report is held for one-sample confirmation: if queued progress then arrives rapidly and cumulative frame/media counters catch up, the watchdog rebases its history instead of restarting healthy capture. Objective output/gap evidence is evaluated before content cadence. GDI uses only those objective checks and does not treat content duplicates as a capture failure. The cadence checks require fullscreen and continuing keyboard, mouse, or controller input; custom borderless/stretched fallback requires the live display device, top-left anchoring, meaningful coverage, and an aspect ratio different from an ordinary smaller native-aspect window. Because that custom-window classification remains heuristic, duplicate-only evidence can perform one boundary-aligned native profile promotion but cannot destructively restart an already-resilient or scaled recorder. An idle/static scene does not trigger recovery. Because sparse HUD changes can hide a stale game surface from duplicate counters, each WGC process also has a bounded 30-minute lifetime.

ClipForge aligns a routine renewal to a completed segment boundary, renews only that process and its audio pipes, and keeps only completed media from a trusted generation. A display transition or confirmed health fault immediately blocks the known-bad generation from export, cancels an in-flight save that already selected that generation, and starts a clean contiguous suffix; saves revalidate under the same gate immediately before commit and never splice across a quarantined head, missing segment, or process generation. Repeated same-size display notifications are coalesced with one trailing renewal so a later HDR/custom-mode event cannot be lost while another refresh or bounded retry is active. A save requested during short maintenance waits instead of being lost, and playback is not closed for a routine renewal. Changed dimensions (or GDI geometry) restart capture, direct-WGC coordinate-only moves stay in place, and an unavailable display stops safely until Windows reports its return. Persistent failure after resilient promotion enters bounded WGC/GDI recovery rather than running indefinitely. Further health failures warn instead of creating a restart loop, while routine age-based renewal remains eligible.
...
An aligned trim with a verified keyframe uses stream copy and is normally near-instant. Arbitrary frame-accurate boundaries still require a second video encode, using a verified hardware encoder when available and a bounded software retry after failure. Trimming can therefore still use noticeable CPU, GPU, disk bandwidth, and temporary disk space for long or high-resolution selections; no editor can guarantee an instant exact encode on every source and driver.
...
Choosing a cloud-synced save folder, such as a OneDrive folder, can cause the operating system or another application to upload saved clips; ClipForge itself does not do so. A forced shutdown can leave temporary files behind. Startup removes current-logon crash residue off the UI thread. It may also remove at most two strictly validated replay roots from inactive Windows sessions older than 24 hours, but never when a matching ClipForge/FFmpeg owner is active or ownership cannot be established. Avoid selecting a replay length larger than the available disk space.
...
- Trimming creates a separate validated MP4. A verified segment/keyframe-aligned range uses fast stream copy, while arbitrary exact boundaries still encode and can take noticeable time or resources after a hardware or software fallback.
