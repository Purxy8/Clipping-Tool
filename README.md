...
For Windows Graphics Capture, ClipForge also reads FFmpeg's lightweight progress counters and watches for two severe failure patterns: an alive process whose progress and segments stop advancing, or a fullscreen foreground session whose advancing CFR stream consists almost entirely of duplicated frames. Duplicate-frame recovery requires at least 75% fullscreen samples plus continuing recent-input evidence across an eight-second window; an idle/static fullscreen scene does not trigger it, and short system overlays do not immediately erase the candidate history. Recovery waits for an in-progress clip save to finish. The first automatic recovery reacquires the same verified WGC strategy; if starvation is detected again, the second and final automatic restart probes the display's actual **Source** geometry and uses it only when a WGC path succeeds, without changing the saved resolution preference. Further detections warn instead of creating a restart loop.
