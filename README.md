...
Source/native WGC, already-fitting presets, and GDI run at BelowNormal CPU/GPU scheduling priority. A fixed-preset WGC path that actually downsizes runs at Normal CPU and GPU priority so its capture/D3D scaling graph can sustain cadence on stretched or custom-resolution games. ClipForge applies the same production-shaped CFR policy to the scaled-WGC capability probe, reapplies the selected live policy after initialization, and repairs drift every 30 seconds. A temporary failed WGC probe is cached for 15 seconds with exponential growth capped at one minute, while an active GDI fallback rechecks WGC on a deliberately quieter five-, ten-, twenty-, then thirty-minute schedule. Successful WGC selections expire after ten minutes, and a confirmed real launch failure invalidates only the exact selection before one fresh retry. Other background probes and thumbnails run at Idle priority, while an explicit trim runs at BelowNormal. Desktop/microphone PCM transfer is bounded and reuses pooled buffers so capture cannot grow an unlimited in-memory queue. Replay retention follows FFmpeg's sequential segment names incrementally instead of repeatedly enumerating and sorting the entire buffer. Valid unchanged clip metadata and short-lived negative probe results are reused from bounded caches, corrupt thumbnail files must pass a real bounded JPEG decode before reuse, and frozen thumbnail decodes use a strong LRU bounded by both key count and total decoded pixels. Fixed presets downscale directly to the smallest even aspect-preserving size inside their bound with Windows Graphics Capture's low-overhead point sampler, bypass scaling when the source already fits, and never pay for a padded fixed canvas. **Source** follows the native capture surface, subject only to encoder-safe even dimensions, and therefore avoids preset downscaling. Source/native WGC retains a two-frame FFmpeg input queue; a fixed preset that actually downsizes uses four smaller output frames so short fullscreen GPU stalls do not starve CFR pacing without restoring the former eight-frame native queue. Hidden, inactive, and tray states cancel unnecessary helpers, release embedded-player sources, keep at most one decoder active, and avoid rebuilding the full UI on background state ticks. Recent and Library bind validated cached cards immediately, then hydrate missing posters through cancellable bounded work. Throughout Buffering and Ready, normal automatic folder discovery and FFprobe work remain suspended; only the bounded autostart recovery described above may refresh missing Recent metadata, and only an active foreground Main window may hydrate missing posters for its validated Recent snapshot. Cached cards stay visible and explicitly requested playback or trimming remains available. A full refresh resumes after replay stops. Player position timers run only while their visible, active window is playing a clip. NVIDIA capture favors game headroom with the fast P2 preset, single-pass encoding, no lookahead, and no B-frames. Capture, save, validation, thumbnail, probe, and trim children are attached to kill-on-close Windows Job Objects so an abrupt ClipForge process termination does not intentionally leave owned media helpers running.

For Windows Graphics Capture, ClipForge reads FFmpeg's lightweight progress counters and watches for an alive process whose progress/segments stop, severe eight-second duplicate starvation, and moderate twenty-second degradation after an aged process has already demonstrated healthy active cadence. Both cadence checks require at least 75% fullscreen and continuing recent-input evidence; an idle/static scene or a game that has always rendered at low cadence does not trigger the moderate tier. Because sparse HUD changes can hide a stale game surface from duplicate counters, each WGC process also has a bounded 30-minute lifetime. ClipForge aligns a routine renewal to a completed segment boundary, renews only that process and its audio pipes, and keeps only completed media from a trusted generation. An unaligned transition or confirmed health recovery quarantines the previous generation rather than risking a clip whose audio continued after video stalled. A generation-tagged request gate prevents a rejected health event or stale UI callback from disabling later scheduled renewals. Same-geometry notifications do not renew capture by themselves; resume, unlock, and other forced transitions still do. A save requested during short maintenance waits instead of being lost, and playback is not closed for a routine renewal. Changed dimensions (or GDI geometry) restart capture, direct-WGC coordinate-only moves stay in place, and an unavailable display stops safely until Windows reports its return. A confirmed scaled-WGC pacing fault falls back to temporary **Source** on the first recovery; native capture waits for the second. Further health failures warn instead of creating a restart loop, while routine age-based renewal remains eligible.
