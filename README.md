...
Every continuous capture path runs its FFmpeg child at BelowNormal CPU priority. On Windows, native/Source WGC and GDI request BelowNormal GPU scheduling priority; a fixed preset that actually scales keeps Normal GPU priority so its D3D graph can meet frame deadlines on stretched/custom-resolution games. ClipForge reapplies the selected policy after initialization and verifies it every 30 seconds. Background probes and thumbnails run at Idle priority, while an explicit trim runs at BelowNormal. Desktop/microphone PCM transfer is bounded and reuses pooled buffers so capture cannot grow an unlimited in-memory queue. Replay retention follows FFmpeg's sequential segment names incrementally instead of repeatedly enumerating and sorting the entire buffer. Valid unchanged clip metadata is reused from a bounded file-identity cache, helper work is serialized within each owner, and frozen thumbnail decodes use a strong LRU bounded by both key count and total decoded pixels. Fixed presets downscale directly to the smallest even aspect-preserving size inside their bound with Windows Graphics Capture's low-overhead point sampler, bypass scaling when the source already fits, and never pay for a padded fixed canvas. **Source** follows the native capture surface, subject only to encoder-safe even dimensions, and therefore avoids preset downscaling. Source/native WGC retains a two-frame FFmpeg input queue; a fixed preset that actually downsizes uses four smaller output frames so short fullscreen GPU stalls do not starve CFR pacing without restoring the former eight-frame native queue. Hidden, inactive, and tray states cancel unnecessary helpers, release embedded-player sources, keep at most one decoder active, and avoid rebuilding the full UI on background state ticks. Recent and Library bind validated cached cards immediately, then hydrate missing posters through cancellable bounded work. Throughout Buffering and Ready, automatic folder discovery and FFprobe work remain suspended; only an active foreground Main window may hydrate missing posters for its already validated Recent snapshot. Cached cards stay visible and explicitly requested playback or trimming remains available. A full refresh resumes after replay stops. Player position timers run only while their visible, active window is playing a clip. NVIDIA capture favors game headroom with the fast P2 preset, single-pass encoding, no lookahead, and no B-frames. Capture, save, validation, thumbnail, probe, and trim children are attached to kill-on-close Windows Job Objects so an abrupt ClipForge process termination does not intentionally leave owned media helpers running.
