...
Every continuous capture path runs its FFmpeg child at BelowNormal CPU priority. On Windows, ClipForge also requests BelowNormal GPU scheduling priority for that capture process; it applies the request again after the WGC/D3D capture graph has initialized and every 30 seconds so late-created graphics work retains the foreground-favoring policy. Auxiliary probe, thumbnail, and replay-coexisting trim helpers run at Idle priority. Desktop/microphone PCM transfer is bounded and reuses pooled buffers so capture cannot grow an unlimited in-memory queue. Replay retention follows FFmpeg's sequential segment names incrementally instead of repeatedly enumerating and sorting the entire buffer. Valid unchanged clip metadata is reused from a bounded file-identity cache, simultaneous Recent/Library requests coalesce to one probe per clip, and frozen thumbnail decodes use a bounded weak cache. Fixed presets downscale directly to the smallest even aspect-preserving size inside their bound with Windows Graphics Capture's low-overhead point sampler, bypass scaling when the source already fits, and never pay for a padded fixed canvas. **Source** follows the native capture surface, subject only to encoder-safe even dimensions, and therefore avoids preset downscaling. Source/native WGC retains a two-frame FFmpeg input queue; a fixed preset that actually downsizes uses four smaller output frames so short fullscreen GPU stalls do not starve CFR pacing without restoring the former eight-frame native queue. Hidden, inactive, and tray states cancel unnecessary helpers, release embedded-player sources, keep at most one decoder active, and avoid rebuilding the full UI on background state ticks. Throughout Buffering and Ready, automatic Recent/Library discovery and FFprobe work remain suspended; only an active foreground Main window may hydrate missing posters for its already validated Recent snapshot. Cached cards stay visible and explicitly requested playback or trimming remains available. A full refresh resumes after replay stops. Player position timers run only while their visible, active window is playing a clip. NVIDIA capture favors game headroom with the fast P2 preset, single-pass encoding, no lookahead, and no B-frames. The capture FFmpeg child is assigned to a per-session Windows Job Object with kill-on-close containment, so an abrupt ClipForge process termination does not intentionally leave that owned recorder running in the background.

For Windows Graphics Capture, ClipForge reads FFmpeg's lightweight progress counters and watches for an alive process whose progress/segments stop, severe eight-second duplicate starvation, and moderate twenty-second degradation after an aged process has already demonstrated healthy active cadence. Both cadence checks require at least 75% fullscreen and continuing recent-input evidence; an idle/static scene or a game that has always rendered at low cadence does not trigger the moderate tier. Because sparse HUD changes can hide a stale game surface from duplicate counters, each WGC process also has a bounded 30-minute lifetime. ClipForge aligns renewal to a segment boundary when possible, renews only that process and its audio pipes, gives FFmpeg a bounded graceful WGC/D3D/COM teardown before kill-on-job-close fallback, keeps every completed replay segment, and discards at most the new in-flight two-second tail. A generation-tagged request gate prevents a rejected health event or stale UI callback from disabling later scheduled renewals. A save requested during this short maintenance window waits instead of being lost, and playback is not closed for a routine renewal. The same buffer-preserving path follows a same-size Windows display/device transition or resume when the selected display returns with compatible source/output dimensions. Changed dimensions (or GDI geometry) restart capture, direct-WGC coordinate-only moves stay in place, and an unavailable display stops safely until Windows reports its return. Routine renewal is unlimited and independent from the two-event fault budget. The first actual health recovery uses the same freshly verified WGC strategy and preserves the ring; a second actual failure probes temporary **Source** geometry without changing the saved preference. Further health failures warn instead of creating a restart loop, while routine age-based renewal remains eligible.
