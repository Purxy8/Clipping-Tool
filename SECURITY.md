...
- Audio named pipes are limited to the current Windows user. Replay roots are separated by Windows logon-session ID. Startup cleanup runs off the UI thread and removes current-session crash residue only after single-instance ownership. A direct-child regular cursor limits each launch to 65 enumerated and 64 fully inspected sibling roots while making durable progress; at most two strictly validated `WindowsSession-<id>` roots inactive for at least 24 hours are removed. ClipForge rechecks ClipForge/FFmpeg ownership immediately before deletion and fails closed for active or unknown owners, reparse points, nested or unexpected content, and oversized trees.
...
- Frame-accurate trim export revalidates and pins the selected source and save root while local FFmpeg reads the file. Handle validation and the Windows free-space query use the normalized pinned local or UNC directory, so these checks remain effective beyond the legacy 260-character boundary. Trim writes a unique partial outside Library discovery, validates the resulting MP4, and atomically commits a separate non-overwriting file; cancellation or failure removes the owned partial and leaves the source unchanged.
