...
A recovery event is tagged with the capture PID and marshalled to `MainWindow`; stale events cannot replace a newer process. Recovery is serialized through the normal capture command gate and save gate. A routine WGC renewal waits for a save and, when capture is still advancing, for the next segment boundary; once that proves the preceding file is finalized it terminates the old job immediately, indexes final output, removes only the newly opened partial tail, recreates audio pipes, and starts the same configuration in the same session directory at the next safe segment number. If capture is already too degraded to reach a boundary, a short bounded graceful attempt falls back to kill-on-job-close containment. The exact pinned FFmpeg file is re-hashed and held open without write/delete sharing through `Process.Start`. Every completed segment remains available, replay remains logically active so an arriving save queues, and routine renewal does not suspend a foreground player. This happens after each 30-minute WGC process lifetime and after a same-size display/device transition or compatible resume; it does not consume the fault budget. Changed source/output dimensions (or GDI's baked geometry) use the full restart path, direct-WGC coordinate-only moves remain in place, and an unavailable selected display is retried for a bounded period before replay stops safely until Windows reports its return. The first starvation/hang event uses the same buffer-preserving path. A second actual health event switches only that running session to native **Source** geometry, freshly probes that output size, and proceeds only if WGC succeeds; it does not persistently change the user's preset. Later health events warn instead of entering a restart loop. A hard hang on the non-WGC compatibility path is killed and reported as `Faulted`.
