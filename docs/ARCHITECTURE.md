...
7. FFmpeg captures the selected display, consumes the PCM inputs, and writes sequential Matroska segments. Direct hardware Windows Graphics Capture runs at Normal priority to reduce the likelihood of capture-feed starvation; compatibility, GDI, and software fallbacks run below normal.
...
- Aspect-preserving output geometry for fixed presets. Each preset is an **up to** width/height bound: it never enlarges a smaller source, aligns scaled dimensions to the nearest encoder-safe even values, and does not add a black padded canvas for square, portrait, 4:3, 16:10, ultrawide, or other custom display modes. The capture path bypasses resizing when the source already fits; GDI uses `fast_bilinear` only when downscaling is required. Source mode rounds odd native dimensions down to even values and otherwise remains unscaled.
