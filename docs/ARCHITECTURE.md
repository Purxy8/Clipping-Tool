...
A health event is tagged with the capture PID and marshalled to `MainWindow`; stale events cannot restart a newer session. Recovery is serialized through the normal capture command gate and is deferred while `Saving` so a stable segment snapshot can complete. The first event stops and restarts the same verified WGC strategy to reacquire its frame pool. A second event switches only that running session to native **Source** geometry, freshly probes that actual output size, and proceeds only if a WGC strategy succeeds; it does not persistently change the user's preset. No more than two automatic restarts are allowed between a manual start or capture-configuration change, and a later detection reports the condition without entering a restart loop. Every recovery clears the old rolling buffer as a normal stop/start does. A hard hang on the non-WGC compatibility path is killed and reported as `Faulted` instead of being left falsely marked as running.
...
- Capture configuration is immutable for the life of a session. The UI automatically performs stop/start when display, resolution, frame-rate, or audio choices change; retention changes apply in place. A debounced Windows display-mode notification refreshes the selected display by device name. Source/native or any change to the resolved encoded dimensions restarts because an H.264 rolling stream cannot change dimensions in place; GDI also restarts for its baked coordinates/input size. Direct WGC keeps coordinate-only moves in place, but every source width/height change now restarts and reacquires its frame pool even if a fixed preset still resolves to the same encoded dimensions. Explicit replay intent survives a mode-transition capture fault so recovery uses the newest geometry.
- Automatic starvation/hang recovery is advisory from the engine and authoritative in the UI lifecycle: the event carries the originating PID, the capture command gate revalidates ownership, saves are allowed to finish, and a per-manual-session budget prevents repeated restart loops. The temporary Source safety mode changes only the recovery session configuration, not persisted settings.
...
- The lightweight watchdog sees FFmpeg's CFR duplicate counter, not decoded pixel hashes. Removing the capture-side FPS filter exposes missing-source-frame duplication to that counter, but a capture source that emits already-timestamped identical pixels can still evade automatic starvation recovery. Target-game decoded-motion testing remains necessary.
