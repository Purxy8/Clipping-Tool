...
A recovery event is tagged with both the capture PID and a generation-specific request ID before it is marshalled to `MainWindow`; stale events cannot replace a newer process or release its request gate. Recovery is serialized through that atomic state machine, the normal capture command gate, and the save gate. Shutdown, dispatcher cancellation, duplicate queueing, fault-budget rejection, preflight failure, and successful completion all resolve only their own generation. A one-shot scheduled display/device refresh arriving behind a health request is stored inside the same locked state and dispatched exactly once after the UI queue clears; another scheduled request already in flight satisfies it. A routine WGC renewal waits for a save and, when capture is still advancing, for the next segment boundary. Once that proves the preceding file is finalized it gives FFmpeg one bounded second to run its orderly WGC frame-handler/session/frame-pool, D3D/COM, muxer, and audio teardown; kill-on-job-close remains the final containment fallback. ClipForge then indexes final output, removes only the newly opened partial tail, recreates audio pipes, and starts the same configuration in the same session directory at the next safe segment number. The exact pinned FFmpeg file is re-hashed and held open without write/delete sharing through `Process.Start`. Every completed segment remains available, replay remains logically active so an arriving save queues, and routine renewal does not suspend a foreground player. This happens after each 30-minute WGC process lifetime and after a same-size display/device transition or compatible resume; it remains eligible even after the independent two-event fault budget is exhausted. Changed source/output dimensions (or GDI's baked geometry) use the full restart path, direct-WGC coordinate-only moves remain in place, and an unavailable selected display is retried for a bounded period before replay stops safely until Windows reports its return. The first starvation/hang event uses the same buffer-preserving path. A second actual health event switches only that running session to native **Source** geometry, freshly probes that output size, and proceeds only if WGC succeeds; it does not persistently change the user's preset. Later health events warn instead of entering a restart loop, without disabling scheduled renewal. A hard hang on the non-WGC compatibility path is killed and reported as `Faulted`.
