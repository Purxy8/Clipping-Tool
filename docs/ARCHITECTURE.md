...
Process priority follows the work being protected. Native WGC/GDI begins BelowNormal so the foreground game, compositor, and input path retain scheduling preference. A path that actually downsizes and a native session promoted after measured recorder pressure use Normal CPU/GPU priority; resilient WGC also uses a four-frame queue. Every WGC and GDI fallback capability probe uses the complete three-second production CFR, scale, encoder, and priority graph. The policy is reapplied when the first progress frame proves graph initialization and checked every 30 seconds for the life of capture. Stable low cadence can cause only the initial native LowImpact WGC session to promote once; it cannot repeatedly restart an already-resilient or scaled recorder. ClipForge is Per-Monitor-V2 DPI aware, and foreground custom-mode classification uses the live monitor device/rectangle, a mismatched stretched aspect ratio, and controller input. An explicit trim uses BelowNormal so it can finish promptly without outranking the foreground game. The software capture fallback also caps encoder worker threads. WASAPI producers keep at most 16 pooled sample blocks, preserve the newest audio if the writer falls behind, and promptly return dropped or shutdown-remnant buffers. The replay monitor follows the segment muxer's strictly increasing file names and keeps an incremental size/index view instead of enumerating and sorting the entire ring on every health poll. Identity-bound valid and short-lived negative probe results use bounded caches, concurrent requests within each owner serialize and double-check those caches before launching a helper, corrupt cached posters must pass a bounded real JPEG decode before reuse, and decoded thumbnails use a strong LRU bounded by both 48 keys and approximately 32 MiB of raw pixels. When a player window is hidden or inactive it cancels unnecessary helper work, closes its media source, stops position timers, avoids full-control work on the one-second state tick, and updates only the small tray/overlay state that is actually visible. These measures reduce contention, disk metadata work, compositor work, and allocation pressure; they cannot guarantee a fixed input-latency result across games, drivers, and hardware.

The continuous capture command emits one machine-readable FFmpeg progress record every quarter second. `CaptureProgressParser` publishes only complete record boundaries, and `CaptureStarvationWatchdog` compares cumulative frame/output-time and CFR duplicate counters without decoding the rolling media. WGC duplicate recovery requires a visible, non-minimized, non-cloaked foreground surface on the selected display plus continuing input evidence. The initial native LowImpact profile has one promotion-only lane for repeated burst stalls or 12 seconds at no more than roughly 55% unique target cadence; scaled and resilient paths do not use that tier. All desktop capture paths can detect below-real-time output and a progress-report gap of at least 2.5 seconds; GDI does not use content duplicate counters. Severe WGC starvation still requires at least 90% duplicates over eight seconds, while aged moderate degradation requires a previously healthy active cadence. One or two transient fullscreen misses can remain in the ratio, while four continuous seconds outside eligibility reset the candidate. A separate hang check requires both FFmpeg progress and segment activity to remain unchanged for seven seconds after startup, and premature completion of either redirected output pump is treated as a broken capture control channel. These thresholds are not a general visual-quality classifier.
...
Thumbnail generation runs asynchronously through FFmpeg with the same local-file/MOV constraints, bounded output capture, cancellation, timeouts, one decode thread, and low-impact process priority. Cached thumbnails use a SHA-256 name bound to the clip path, metadata, Windows volume serial, and 128-bit file ID. A validated, non-writable/non-deletable clip handle and a pinned save-root handle remain open while FFmpeg reads by path and until the cache entry is committed, preventing a same-path file swap during decoding without copying the complete video. Atomic replacement prevents a crash from leaving a trusted partial entry, while cleanup pins and validates the cache-directory and thumbnail handles before deletion; loaded clips also prune their legacy v1.2 cache key. Frozen decoded JPEGs are retained in a strong least-recently-used cache bounded by key count and total decoded pixels, avoiding repeated GC-triggered disk decodes without unbounded image memory. The library is a local view over existing files; it does not copy or upload clips.

The large `MediaElement` surfaces provide play/pause, restart, clickable/draggable and keyboard seeking, elapsed/total time, 10-second backward/forward skips, mute, and volume. Playback is independent of steady replay capture. The recent gallery loads only the persisted 4/8/10/15-item limit, derives card width from both the requested limit and the number of clips actually found so a short row fills its viewport, displays the already-enumerated file size, and scrolls larger sets horizontally. **Library** loads up to the newest 100 validated clips into a recycling `VirtualizingStackPanel`; its All/Normal/Trimmed filter is applied to the classified candidates, and only the selected item is connected to a decoder. A Windows-autostart launch validates and binds the bounded Recent snapshot before replay starts. Throughout Buffering and Ready, normal Recent/Library folder discovery and FFprobe metadata work remain suppressed. If that autostart preload timed out or failed, a visible, active, steadily Ready Main window may run a two-second cached-poster/metadata-only recovery no more often than every 30 seconds; save-directory and requested-count snapshots are rechecked before binding, and focus loss or a critical capture transition cancels it. Main may also generate missing posters only for an identity-bound in-memory snapshot; generation is serialized, Idle-priority, budgeted, snapshot-version checked, and cancelled under the same conditions. Existing cached cards and posters remain visible, and explicit playback, seeking, and trim operations against a selected existing clip remain available. A full refresh resumes after replay stops. The player is attached on explicit selection/play, shows the selected cached poster while deferred, starts muted, and records an explicit audio opt-in if the user unmutes; this matters because normal playback audio can be captured by desktop loopback. Because WPF manual media graphs do not always open from `Source` assignment alone, Library starts a muted prime and pauses in `MediaOpened` unless autoplay was requested. Short replay transitions that start, save, or stop capture suspend presentation, cancel refresh work, and release the decoder; the chosen clip and paused position are restored afterward. Hiding, deactivating, or closing either player clears its source and timer, and activation restores a remembered position without autoplay. Newer refresh requests cancel superseded probe/thumbnail work. Thumbnail JPEGs use `BitmapCacheOption.OnLoad` and frozen in-memory images, allowing deterministic cache files to be removed immediately. Right-click reveal and deletion revalidate ClipForge ownership and stable Windows volume/file identity; deletion rejects multi-link files, closes the selected media source, and uses a validated Windows handle. `UiMotionService` limits optional motion to short opacity/translation transitions, disables it for High Contrast, reduced-motion, or software-rendering conditions, and removes completed animation clocks.

### Trimming a Library clip

The latest-clip player and Library expose a two-handle range over the selected clip's timeline. Moving a handle debounces intermediate decoder seeks and performs one exact seek when the drag completes. An arithmetically segment-aligned range uses near-instant stream copy only after FFprobe confirms an H.264 keyframe at the requested start. The copied MP4 is read back and timeline-validated; an unreadable, late-starting, wrong-duration, or otherwise invalid copy is removed and retried through exact encoding. Every other range is encoded directly for exact committed boundaries. Standard and replay-coexisting exports prefer a runtime-verified hardware encoder, retry safely with software after a hardware failure, and run at BelowNormal priority. Replay coexistence does not impose artificial real-time input pacing, while transient start, save, and stop presentation states still suspend player/trim interaction until the capture transition completes.

Before export, the selected clip and save root are revalidated and held through pinned Windows handles. Those handle operations use normalized extended-length paths so a safe direct child remains verifiable beyond legacy `MAX_PATH`. The local FFmpeg child receives a fully resolved trusted executable path, individual argument-list entries, a forced MOV/MP4 input demuxer, and a `file`-only protocol whitelist; it runs without a command shell at BelowNormal priority and is attached to a kill-on-close Job Object. A uniquely named partial is written in the destination directory, remains outside Library discovery, and is removed on cancellation or failure. ClipForge probes the completed partial and revalidates the pinned source/root before atomically committing it under a non-overwriting trimmed name. The original is never modified in place.
...
Routine process-age, display/device, resume, unlock, and reconnect renewals use a PID-scoped service coordinator that rejects stale work and cannot overlap a save, another recorder, or teardown. A routine WGC renewal waits for the next completed segment boundary, re-verifies and pins the exact FFmpeg executable through `Process.Start`, performs bounded orderly teardown, discards the newly opened partial tail, recreates audio pipes, and continues at the next unused segment number. The first replacement segment is quarantined. Export and the displayed available duration use only the newest contiguous trusted suffix; they never skip that head, an unexplained number gap, or a blocked generation to splice different process timelines into one MP4. Same-size display events are epoch-coalesced, including a trailing event that arrives during another renewal. Changed source/output dimensions (or GDI's baked geometry) use the full restart path, direct-WGC coordinate-only moves remain in place, and an unavailable selected display is retried for a bounded period before replay stops safely.

Native LowImpact WGC can perform one non-destructive, boundary-required promotion to the Resilient queue/priority profile. If the boundary is not observed, the old process remains active and the promotion is retried instead of invalidating its buffer. Objective throughput/gap/hang evidence can supersede that deferred maintenance. Custom/stretched duplicate-only evidence cannot destructively restart a scaled or already-resilient recorder. Persistent proven failure enters a two-event WGC/GDI recovery budget; an accepted attempt consumes its slot even when engine verification defers the replacement, and exhaustion stops replay rather than leaving a running but unexportable generation. A hard GDI hang is killed and reported as `Faulted`. A capability-probed GDI session retains its active profile during refresh or failed WGC-promotion rollback, keeps its background WGC opportunity, and runs the real re-probe only after all recent foreground input has become idle; input resuming during the probe cancels it.
...
FFmpeg and FFprobe are launched by fully resolved executable path with `UseShellExecute=false`, no command shell, and individual argument-list entries. Normal builds accept only the pinned private/bundled executable hashes, and capture renewal forces a fresh SHA-256 calculation plus canonical-path equality rather than accepting the normal file-stamp verification cache. The continuous capture FFmpeg and capability/media helpers are attached immediately to private kill-on-close Windows Job Objects; if capture containment fails, startup fails instead of running an uncontained recorder. Media probe, thumbnail, and trim processes accept only ClipForge-generated top-level candidates, revalidate file identity immediately before use, force MOV/MP4 demuxing with the local `file` protocol, cap helper runtime and diagnostic output, honor cancellation, and terminate their child process on timeout. Handle-based source/output validation uses normalized extended-length Windows paths, so the same identity and direct-child checks continue to work beyond the legacy 260-character boundary. Trim export additionally pins the source and destination root, queries free space through that normalized pinned directory (including UNC roots), writes an undiscoverable unique partial, validates it before an atomic non-overwriting commit, and revalidates the original before any optional deletion. WASAPI named pipes are restricted to the current user. Replay roots are separated by Windows logon-session ID. Startup maintenance runs off the WPF thread and `StartAsync` awaits it before capture. Current-session crash residue is removed after single-instance ownership; at most two strictly validated `WindowsSession-<id>` sibling roots inactive for at least 24 hours may also be removed per launch. A durable regular-file cursor partitions large sibling sets by decimal session-ID suffix and caps each launch at 65 enumerated and 64 fully inspected candidates. ClipForge persists the next bucket before deletion, rechecks ClipForge/FFmpeg ownership immediately before each deletion, and fails closed for active or unknown owners, reparse points, unexpected/nested content, or oversized trees. The older unscoped `Buffer/session-*` migration remains separately bounded. User-facing Open actions accept only fully qualified existing local paths. The optional FFmpeg installer uses bounded download/extraction, a pinned HTTPS archive digest, and pinned hashes for both extracted executables.
...
- Capture configuration is immutable except during an internal same-configuration WGC process renewal. The UI performs a full stop/start when display, resolution, frame-rate, or audio choices change; retention changes apply in place with an O(1) state update, immediate available-duration clamping, and at most 64 deletion attempts per monitor refresh while protected export segments remain untouched. Trusted oldest-excess and untrusted quarantine candidates receive independent fair shares, unused capacity is lent to the other class, and service-persistent segment-ID cursors prevent locked files from starving later candidates without ever selecting the newest retained tail. A debounced Windows display notification re-resolves the selected device. Source/native or any source-size change fully restarts because an H.264 ring cannot change dimensions in place; GDI also restarts for baked coordinates. Direct WGC keeps coordinate-only moves, renews for a same-size mode/device event, and renews after resume from sleep.
...
- Trim export is single-flight. It can coexist with steady replay through a BelowNormal-priority hardware path when runtime verification succeeds, with a bounded software fallback after failure. Transient start/save/stop states suspend presentation and trim interaction, and entering trim cancels background Library discovery/thumbnail hydration.
...
- A segment-aligned trim with a verified keyframe uses stream copy and is normally near-instant. Other trim ranges require an exact encode and can still take noticeable time or resources, especially after a hardware-encoder failure falls back to software.
