...
2. Keep replay steadily Buffering/Ready. Confirm Library remains browsable, its list paints immediately with placeholders/cached images, only the bounded visible/selected missing thumbnails hydrate while ClipForge stays foreground, selecting a clip opens one foreground decoder, playback and seeking work, and playback starts muted. Move focus back to the game and confirm outstanding thumbnail work stops. If desktop-audio capture is enabled, explicitly unmute once and confirm the UI warns that playback sound can enter the rolling buffer.
