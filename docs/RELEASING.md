...
dotnet run --project $smokeProject -c Release --no-build -- --length-matrix --artifacts $smokeRoot
dotnet run --project $smokeProject -c Release --no-build -- --recorder-fast-path-smoke --save-seconds 12 --audio --artifacts $smokeRoot
dotnet run --project $smokeProject -c Release --no-build -- --recorder-service-smoke --resolution 1080p --fps 60 --audio --save-seconds 12 --force-segment-fallback --artifacts $smokeRoot
dotnet run --project $smokeProject -c Release --no-build -- --resolution 720p --fps 60 --audio --microphone --artifacts $smokeRoot
dotnet run --project $smokeProject -c Release --no-build -- --resolution 1080p --fps 60 --audio --microphone --save-seconds 30 --artifacts $smokeRoot
...
dotnet run --project $smokeProject -c Release --no-build -- --wgc-matrix --matrix-resolutions source,720p,1080p,1440p,2160p --fps 60 --audio --microphone --force-wgc --motion-validation --self-motion-surface --artifacts $smokeRoot
```

The smoke harness fails on an unexpected duration, resolution, average frame rate, frame-count floor, audio-stream count, capture priority, excessive normalized CPU, or excessive working set. It also inspects packet timestamps: video frame spacing must stay within the configured FPS budget, audio DTS must remain monotonic, and audio/video stream durations must stay aligned across two-second replay-buffer joins. The harness derives the initial process policy from the selected backend and geometry: any path that actually downsizes must remain Normal on CPU and GPU; Source/native WGC, already-fitting presets, and native GDI begin BelowNormal. A native session deliberately promoted by the runtime cadence watchdog must instead expose its Normal resilient policy (plus the four-frame queue for WGC) and retain that policy across automatic display-mode restarts. Verify the selected pair after launch, after graph initialization, after renewal, and after the 30-second drift-repair boundary. The desktop-independent `--concat-smoke` case builds three real 1920x1080 60 FPS AAC-backed segments and must complete with no join gap larger than 25 ms.

Nominal frame rate and valid timestamps are not sufficient evidence that a live capture contains fresh frames: CFR output can repeat a stale surface while still reporting exactly 60 FPS. Every interactive WGC release run must therefore keep `--motion-validation` enabled and inspect decoded-frame hashes/duplicate transitions for each Source and fixed-preset output. For the controlled composed-desktop baseline, add `--self-motion-surface`; the matrix parent runs a topmost animated WinForms surface on a dedicated STA thread over the exact captured display, waits for an explicit ready signal, and closes it within a bounded `finally` cleanup. Omit that switch and use visibly changing target-game content for exclusive-fullscreen/custom-resolution checks. Retain the motion report with the release evidence, and fail the release candidate if a moving scene becomes an extended sequence of identical decoded frames.

`--renew` defaults to two consecutive replacements (`--renew-count 1..12` overrides it). Add `--scheduled-renew` to exercise the service-owned background coordinator used by the 30-minute production maintenance path instead of calling the refresh API directly. It must prove a new PID and exactly one newer generation each time, monotonically increasing/non-reused segment numbering, retention of every previously completed file, and completion of the first trusted replacement segment plus its successor. A documented gap at each known quarantined generation-head ID is expected: ClipForge deliberately removes the first replacement segment after its successor proves steady capture; any other gap fails the run. Also verify that export and the UI stop at that gap and use only the newest contiguous trusted suffix rather than silently joining pre/post-renewal process timelines. Retain the schema-v4 `renewal-<run>.json` report with the release evidence; it records UTC request/completion bounds, each old/new PID, replacement generation, quarantined head, first trusted and successor segment IDs, rollover timestamps, and bounded host/replacement resource samples. Its API duration is an upper bound that includes time spent waiting while the old process is still capturing. Valid MP4 timestamps after concat do **not** by themselves prove zero wall-clock blackout, so use a continuously advancing visible clock/frame counter on the target display when evaluating the handoff and document any observed jump.

`--resolution-matrix` first checks every Source/preset geometry combination for standard, 16:10, 4:3, ultrawide, super-ultrawide, square, portrait, and odd-sized inputs, then performs a curated set of real 30/60 FPS FFmpeg encodes. Add `--matrix-exhaustive` to encode every source/preset pair. `--length-matrix` checks every configured 30-second through one-hour replay length with deterministic video/audio timelines; `--save-seconds` adds a representative live save at a selected duration. The length matrix uses a tiny synthetic seed, so its export time validates engine/timeline overhead rather than real high-bitrate disk throughput. Fixed presets are **up to** bounds: expected outputs stay even, keep the source aspect ratio, never upscale, and never use a padded fixed canvas. In WGC, Source and an already-fitting preset remain on the native surface path. A reduced NVIDIA path must use native-even `gfxcapture` followed by `scale_cuda`; compatibility-transfer graphs use system-memory `fast_bilinear`, and directly compatible QSV/AMF graphs retain their probed source point scaling. `--wgc-matrix` runs the chosen presets sequentially against the selected live Windows display. It must be repeated after switching the target game among borderless/fullscreen modes and any custom internal resolution because the harness cannot change those modes itself. Confirm that a source-size/fullscreen mode transition restarts capture and reacquires WGC even when the selected fixed preset's final encoded dimensions do not change; coordinate-only monitor moves should not force that restart.

Record the selected capture strategy and measured results. `--force-gdi` and `--force-wgc` are internal smoke-only overrides that keep the runtime-verified encoder while isolating the requested capture backend; matrix child runs inherit those overrides together with the audio, microphone, motion, pruning, and probe-diagnostic flags. Neither override is available in the production application. A synthetic or successful WGC/fallback matrix on one PC does not reproduce an affected GPU, driver, exclusive-fullscreen composition path, game's internal resolution, or input-latency conditions. Do not claim that every resolution or game is lag-free without validation on the affected or equivalent target hardware.
...
For the progress watchdog, leave a static fullscreen surface idle well beyond eight seconds and confirm that it does not trigger recovery. Briefly show and dismiss a system overlay and confirm that one or two transient fullscreen-probe misses do not erase a genuine candidate, while four continuous seconds outside eligibility reset it. Confirm that an ordinary smaller native-aspect borderless window at the monitor origin is not classified as stretched fullscreen. On an affected target where starvation can be reproduced safely, retain the diagnostic evidence and confirm that duplicate-plus-below-real-time output is classified by the objective throughput lane, the first ambiguous custom/native LowImpact event promotes the same verified path only at a completed segment boundary, an already-resilient or scaled custom path ignores duplicate-only evidence, and GDI throughput failure never enters the WGC-only Source-safety branch. While GDI is active, keep playing past its re-probe deadline and confirm that no competing probe starts until the game is idle or alt-tabbed. Trigger a fault during Save and confirm the in-flight output is canceled before commit and the bad generation cannot be selected again. No third automatic fault restart may occur. A test that cannot reproduce starvation must be recorded as such rather than represented as proof that the recovery path ran.
...
2. Keep replay steadily Buffering/Ready. Confirm Library remains browsable and cached cards/posters remain visible, with no normal automatic folder discovery or FFprobe process for the entire replay interval. Start once through Windows autostart with four or more existing clips, then open Main and confirm all configured Recent cards are already listed. Also induce a preload timeout: while Main is visible, active, and steadily Ready, confirm one cached-poster/metadata-only recovery runs for at most two seconds and retries no more often than every 30 seconds; changing the save directory/count, deactivating Main, or entering Start/Save/Stop must cancel or reject its stale result. If a poster is missing, confirm at most one Idle-priority thumbnail helper hydrates only the validated foreground Recent snapshot. Select an already listed clip and confirm explicit playback and seeking still work, only one foreground decoder opens, and playback starts muted. If desktop-audio capture is enabled, explicitly unmute once and confirm the UI warns that playback sound can enter the rolling buffer. Stop replay and confirm one full Recent/Library refresh resumes, discovers newly saved clips, and fills missing metadata and thumbnails.
...
8. Confirm an aligned/keyframe-verified selection takes the stream-copy path, while an arbitrary selection uses the verified hardware encoder at Below Normal priority and safely retries with software after an induced hardware failure. Record duration and resource observations for 720p, 1080p, square/custom, and Source/high-resolution inputs; these measurements describe only the test hardware and are not proof of zero latency everywhere.
...
Do not publish a frame-accuracy claim from UI observation alone. Record the input, selected timestamps, output duration/stream details, and FFprobe result in the release verification notes. Do not claim zero lag: an aligned/keyframe-verified trim can be near-instant through stream copy, but every arbitrary exact boundary still requires an encode and can use noticeable CPU/GPU, disk bandwidth, and temporary space.
...
4. Replay-time trim uses verified keyframe copy or the hardware-first exact-encode path with bounded software fallback, keeps both files by default, and deletes only an explicitly confirmed, identity-revalidated original. Start/save/stop transitions suspend presentation safely and restore it afterward.
